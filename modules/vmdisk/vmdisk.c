 ...
 */
void vmem_disk_invalidate(struct timer_list *timer)
{
	struct vmem_disk_dev *dev = container_of(timer, struct vmem_disk_dev, timer);
...
static void setup_device(struct vmem_disk_dev *dev, int which)
{
	 ...
	 */
	timer_setup(&dev->timer, vmem_disk_invalidate, 0);
